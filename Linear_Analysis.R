...
# Descriptive statistics overrall
mean(df_lab$perceivedrecognition)
sd(df_lab$perceivedrecognition)

mean(df_lab$actualrecognition)
sd(df_lab$actualrecognition)

mean(df_lecture$perceivedrecognition)
sd(df_lecture$perceivedrecognition)

mean(df_lecture$actualrecognition)
sd(df_lecture$actualrecognition)
...
# Run four model checks: (1) all predictor variables excluding students receiving 0 nominations, (2) just the three main variables excluding students receiving 0 nominations, (3) all predictor variables reducing (at random) the number of students receiving 0 nominations, and (4) just the three main variables reducing (at random) the number of students receiving 0 nominations
...
actualplot<-ggplot(aggregate_props,aes(x=context, y=prop-expected,fill=gender,ymin=conf_low-expected, ymax=conf_high-expected)) +
  ...
  ylab("Difference Between Observed\n and Expected Proportions\n of Received Recognition") + xlab("")+theme_classic() +theme(axis.title = element_text(size=16,color='black',face="bold"),axis.text = element_text(size=16,color='black'),axis.text.x=element_text(face="bold")) +
...
df<-data.frame(coefficient=c(-0.238,0.074,-0.012,0.441,-0.16,-0.104,-0.166,-0.04,-0.219,-0.103,-0.228,-0.331,-0.435,0.179,0.097,-0.162,-0.472,0.205,-0.108,0.043,-0.3,0.11,-0.014,0.537,-0.201,-0.202,-0.194,-0.152,-0.212,-0.047,-0.194,-0.035,-0.122,0.134,0.03,-0.274,0.899,-0.167,-0.453,-0.044),
               ...
               term=c("Gender", "Received Recognition", "Gender \u00D7 Received Recognition", "Physics", "Life sciences or biology", "Other physical sciences", "Other major","Unknown major","Second year","Third year","Fourth year","Other year","Unknown year","White","Asian","Hispanic","Native Hawaiian", "American Indian","Middle Eastern", "Black","Gender", "Received Recognition", "Gender \u00D7 Received Recognition", "Physics", "Life sciences or biology", "Other physical sciences", "Other major","Unknown major","Second year","Third year","Fourth year","Other year","Unknown year","White","Asian","Hispanic","Native Hawaiian", "American Indian","Middle Eastern", "Black"),
...
df$term<-factor(df$term,levels=rev(c("Gender", "Received Recognition", "Gender \u00D7 Received Recognition", "Physics", "Life sciences or biology", "Other physical sciences", "Other major","Unknown major","Second year","Third year","Fourth year","Other year","Unknown year","White","Asian","Hispanic","Native Hawaiian", "American Indian","Middle Eastern", "Black")))


modelplot<-ggplot(df[df$term=="Gender"|df$term=="Received Recognition"|df$term=="Gender \u00D7 Received Recognition",], aes(x=term, y=coefficient,shape=context)) +geom_abline(intercept =0, slope = 0, color="grey20", size=0.6)+xlab('Semester')+ geom_point(position=position_dodge(width=0.8),size=5) + geom_errorbar(aes(ymin=coefficient-error, ymax=coefficient+error), width=.2, position=position_dodge(0.8),size=0.8) + theme_classic() +theme(axis.title = element_text(size=16,color="black",face="bold"),axis.text = element_text(size=16,color="black"))+scale_shape_manual(values = c(15,17))+theme(strip.text.x = element_text(size = 16))+theme(strip.text.y = element_text(size = 16)) + ylab('Coefficient Estimate for Perceived Recognition')+xlab('Predictor Variable')+ theme(legend.title = element_blank(),legend.text = element_text(size=16,color="black"),strip.background = element_rect(colour="black", size=1,fill="white"),legend.position=c(0.2,0.9))+ labs(color = "")+ylim(-0.5,0.5)+theme(legend.position="bottom",legend.spacing.x = unit(0.3, 'cm'))+ theme(strip.text.x = element_text(
  size = 16,face='bold'),
  strip.text.y = element_text(
    size = 16),strip.background = element_rect(
      color="gray20", fill="white", size=1, linetype="solid"),panel.border = element_rect(color = "gray20", fill = NA, size = 1))+coord_flip()+theme(text=element_text(family="DejaVu Sans"))
...
all.props.split<-ggplot(all.props,aes(x=course, y=prop-expected,fill=gender,ymin=conf_low-expected, ymax=conf_high-expected)) +
  ...
  ylab("Difference Between Observed\n and Expected Proportions\n of Received Recognition")  + xlab("Course")+theme_classic() +
  theme(axis.title = element_text(size=16,color='black',face="bold"),axis.text = element_text(size=16,color='black')) +
  ...
  theme(panel.border = element_rect(color = "gray20", fill = NA, size = 1))+facet_grid(~context,scales="free")+theme(text=element_text(family="DejaVu Sans"))
...
perceived_by_course<-ggplot(df.distributions, aes(x=course, y=mean_perceived)) +
  ...
  theme(axis.ticks.x=element_blank())+scale_colour_manual(values=c("#6dc6b1","#2c7fb8"))+xlab("Course")+ylim(2,5)+theme(text=element_text(family="DejaVu Sans"))
