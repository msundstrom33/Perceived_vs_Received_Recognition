...
df$term<-factor(df$term,levels=rev(c("Woman", "Received recognition", "Woman*Received recognition", "Physics", "Life sciences or biology", "Other physical sciences", "Other major","Unknown major","Second year","Third year","Fourth year","Other year","Unknown year","White","Asian","Hispanic","Native Hawaiian", "American Indian","Middle Eastern", "Black")))


modelplot<-ggplot(df[df$term=="Woman"|df$term=="Received recognition"|df$term=="Woman*Received recognition",], aes(x=term, y=coefficient,shape=context)) +geom_abline(intercept =0, slope = 0, color="grey20", size=0.6)+xlab('Semester')+ geom_point(position=position_dodge(width=0.8),size=5) + geom_errorbar(aes(ymin=coefficient-error, ymax=coefficient+error), width=.2, position=position_dodge(0.8),size=0.8) + theme_classic() +theme(axis.title = element_text(size=16,color="black",face="bold"),axis.text = element_text(size=16,color="black"))+scale_shape_manual(values = c(15,17))+theme(strip.text.x = element_text(size = 16))+theme(strip.text.y = element_text(size = 16)) + ylab('Coefficient Estimate')+xlab('Predictor Variable')+ theme(legend.title = element_blank(),legend.text = element_text(size=16,color="black"),strip.background = element_rect(colour="black", size=1,fill="white"),legend.position=c(0.2,0.9))+ labs(color = "")+ylim(-0.5,0.5)+theme(legend.position="bottom",legend.spacing.x = unit(0.3, 'cm'))+ theme(strip.text.x = element_text(
  size = 16,face='bold'),
  strip.text.y = element_text(
    size = 16),strip.background = element_rect(
