...
lineplot.all<-ggplot(plot.main.figure[!plot.main.figure$n==1,], aes(x=actualrecognition, y=mean_perceived,group=gender)) +
  ...
  scale_colour_manual(values=c("#6dc6b1","tomato2","#2c7fb8"))+
...
df<-data.frame(coefficient=c(-0.237,0.074,-0.011,-0.302,0.108,-0.012),
               ...
               term=c("Gender", "Received Recognition", "Gender \u00D7 Received Recognition","Gender", "Received Recognition", "Gender \u00D7 Received Recognition"),
               context=c("Lab","Lab","Lab","Lecture","Lecture","Lecture")
)


df$term<-factor(df$term,levels=rev(c("Gender", "Received Recognition", "Gender \u00D7 Received Recognition")))
